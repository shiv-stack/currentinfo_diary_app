...
class RemoteConfigService {
      ...
      await _remoteConfig.setConfigSettings(
        RemoteConfigSettings(
          ...
          minimumFetchInterval:
              Duration.zero, // Set to zero for immediate updates during testing
        ),
      );
  ...
  Future<bool> shouldForceUpdate() async {
    try {
      ...
      debugPrint("RemoteConfig: Local Version: $localVersion");
      debugPrint("RemoteConfig: Min Version: $minVersion");
      debugPrint("RemoteConfig: Force Update Active: $isForceUpdateActive");

      if (!isForceUpdateActive) return false;

      final isLower = _isVersionLower(localVersion, minVersion);
      debugPrint("RemoteConfig: Should Update: $isLower");
      return isLower;
    } catch (e) {
  ...
  bool _isVersionLower(String current, String required) {
    try {
      ...
      final List<int> curVer = currentParts[0]
          .split('.')
          .map(int.parse)
          .toList();
      final List<int> reqVer = requiredParts[0]
          .split('.')
          .map(int.parse)
          .toList();
